...
**Resolved:**
- ✅ Syntax error (curly apostrophe)
- ✅ Bible data not loading
- ✅ Annotations not displaying
- ✅ Chapter navigation not refreshing
- ✅ selectedVerse being cleared on save
- ✅ Scroll position too aggressive
- ✅ populateBookSelector error after removing dropdown
- ✅ Icons overlapping with highlights
- ✅ Tag text unreadable in some themes (Github Dark)
- ✅ Header using extra colour outside 8-colour system
...
**Tested & Working:**
- ✅ All 24 themes
- ✅ Auto-contrast on highlights and tags
- ✅ Visual navigation (book/chapter grids)
- ✅ Margin icons separate from highlight area
- ✅ Theme-aware tag colour presets
- ✅ Supabase connection and RLS
- ✅ Authentication flow
- ✅ All 66 books loading
...
**v1.1.0 Remaining:**
1. ✅ ~~Annotation visibility modes~~
2. ✅ ~~Multiple annotation sets~~
3. **Export annotations** (JSON download)

**Then v1.2.0:**
- Font controls (size adjustment, OpenDyslexic option)
- Full color customization

**Completed this session (2025-11-20):**
- ✅ Annotation visibility modes (On/Subtle/Off):
  - On: full color (default)
  - Subtle: greyscale icons, highlights hidden
  - Off: everything hidden including inline menus
  - Dropdown in settings menu
  - Saved to localStorage
- ✅ Multiple annotation sets (Study, Church, Personal):
  - Set switcher in navigation bar (stack icon + current set name)
  - Modal to view/create/switch/delete sets
  - Default sets: Study, Church, Personal
  - Complete isolation - annotations filtered by active set
  - Supabase integration with cascade delete
  - Can't delete last set
- ✅ Bug fixes:
  - Fixed highlight CSS selectors (target .verse-body not .verse)
  - Fixed annotation mode not applying after chapter load
  - Fixed tag color picker closing submenu (stopPropagation)

**Completed previous session (2025-11-19):**
- ✅ Full annotation system redesign:
  - Moved tag/note icons below verse (outside highlight area)
  - Built inline annotation menu: Highlight | Note | Tag | Copy
  - Highlight submenu: H1-H6 colours + X clear, U1-U6 underline + X clear
  - Tag submenu: active tags, existing tags, new tag with colour picker
  - Note submenu: textarea with Delete | Save, CMD+Enter shortcut
  - Copy verse to clipboard with visual feedback
- ✅ Theme selection page redesign:
  - Full visual previews with H1-H6 highlights and tag colours
  - Sample verses showing highlighted, tagged, and plain text
  - Theme persistence fixed (pageshow handler for bfcache)
  - 19 themes in 4 groups (WCAG AAA, Accessibility, Light, Dark)
- ✅ CSS split to separate file (scripture-scribbles-v1.1.css)
- ✅ Mobile hamburger menu fix (touch events, tap delay prevention)

**Completed previous session:**
- Header refactor (hamburger menu, simplified nav)
- Settings menu with theme link, bug report, sign out
- Book name clickable for navigation
- Fixed Supabase 406 errors (.single() → .maybeSingle())
- Fixed session stale after tab switch (visibility change handler)
- Fixed duplicate auth loads on session refresh
...
**Key Architecture:**
- Verse structure: `.verse > .verse-body + .verse-indicators`
- Highlights/underlines apply to `.verse-body` only
- Icons in `.verse-indicators` (outside highlight area)
- Inline annotation menu: 4 buttons (Highlight, Note, Tag, Copy) with submenus
